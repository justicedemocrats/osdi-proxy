module.exports = async (api) => {
   ...
   * Search in between
   */
  const search_results = await Promise.all(missing_ids.map(id => api.searchEvents({
    date_start: '2000-01-01 00:00:00',
    event_id: id
  })))
  ...
  while (found != 0) {
    ...
    let nexts = await Promise.all(new Array(search_ahead).fill(null).map(async (_, inc) => {
